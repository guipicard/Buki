...
namespace buki {
    ...
    public:
        ...
        virtual void GetTextSize(const std::string& text, size_t fontId, int* w, int* h) = 0; // Calcule les dimensions d'une chaîne de texte rendue.

        virtual void DrawCircle(float x, float y, float r, const Color& color) = 0; // Dessine un cercle avec un rayon et une couleur spécifiés.

        virtual void DrawPoint(float x, float y, const Color& color) = 0; // Dessine un point à une position spécifiée.

        virtual void GetWindowSize(int* w, int* h) = 0; // Récupère les dimensions de la fenêtre de rendu.
    };
}
